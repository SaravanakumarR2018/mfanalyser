...
| Portfolio chart | Canvas pixels, point metadata, display-only carry-forward values on non-observation calendar dates, period buttons, zoom, X endpoints, draggable/keyboard window, invested-series toggle, pointer tooltip |
...
| Drawer NAV chart | Default invested-period view and an off-by-default compact full-history switch, calendar-day carry points distinguished from official observations, preserved CAS investment markers, atomic loading transition, retry/fallback, request reuse, period/range controls, observation metadata, rendered pixels, responsive containment, empty transaction state |

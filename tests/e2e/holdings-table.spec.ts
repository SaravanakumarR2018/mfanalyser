...
for (const width of [320, 390, 768, 1440]) {
  test(`comparison headings and fund names remain pinned at ${width}px`, async ({ page }) => {
    ...
    const dimensions = await region.evaluate((element) => ({ height: element.clientHeight, content: element.scrollHeight }));
    expect(dimensions.height).toBe(dimensions.content);
    await region.evaluate((element) => window.scrollTo(0, window.scrollY + element.getBoundingClientRect().top + 180));
    await expect.poll(async () => (await header.boundingBox())!.y).toBeCloseTo((await page.locator(".dashboard-header").boundingBox())!.height, 0);
    ...
    await region.locator(".fund-name").last().scrollIntoViewIfNeeded();
    await expect(region.locator(".fund-name").last()).toBeInViewport();
    expect(await region.evaluate((element) => element.scrollTop)).toBe(0);
  });
}
...
test("finger swipes scroll the page and table columns without opening fund details", async ({ page, browserName, isMobile }) => {
  ...
  const swipe = async (dx: number, dy: number) => {
    ...
    const y = Math.min(700, box.y + box.height * .7);
  ...
  await swipe(0, -150);
  await expect.poll(() => page.evaluate(() => window.scrollY)).toBeGreaterThan(pageTop + 60);
  expect(await region.evaluate((element) => element.scrollTop)).toBe(0);

...
| Transactions and NAV chart | Same-day aggregation, purchases vs redemptions, official vs transaction NAV provenance, latest marker, earliest-published full-history range, invalid input filtering, and display-only calendar carry points that never become official observations |
...
| India inflation context | Browser-only World Bank URL contract, response identity/schema validation, finite annual observations, year deduplication, chronological sorting, latest-30 selection, abort signal forwarding, and safe unavailable responses |
...
| Chart/model calculations | Empty/constant scale, non-finite inputs, calendar-day carry provenance, fund stacks, closed funds, contribution reconciliation, deterministic sorting and immutability |
